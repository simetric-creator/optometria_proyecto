      ...
      case 'acuity':
        document.querySelectorAll(`input[name="acuity-${test.id}"]`).forEach(inp => {
          inp.addEventListener('change', () => {
            ...
            if (isNoResponse) {
              answers[test.id] = { col: 'NR', row: 0, noResponse: true, snellen: 'N/R' };
            } else {
              const col = inp.dataset.col;
              const row = parseInt(inp.dataset.row, 10);
              const cell = (test.columns[col] || []).find(r => r.row === row);
              const snellen = cell?.snellen
                || (typeof window.getAcuitySnellen === 'function' ? window.getAcuitySnellen(col, row) : '');
              answers[test.id] = { col, row, noResponse: false, snellen };
            }
            markComplete(getCurrentStep());
          });
        });
